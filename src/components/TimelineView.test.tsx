...
describe('TimelineView', () => {
  ...
  it('should render timeline with events', () => {
    ...
    expect(screen.getByText('Activity Timeline (2 events)')).toBeInTheDocument();
  ...
  it('should sort items by date (newest first)', () => {
    ...
    const titles = screen.getAllByRole('link').filter(link => 
      link.getAttribute('href')?.includes('/issues/') || link.getAttribute('href')?.includes('/pull/')
    );
    ...
    const commentItems: GitHubItem[] = [
      {
        ...
        closed_at: undefined,
        labels: [
          { name: 'question', color: 'cc317c', description: 'Question or discussion' }
        ],
        repository_url: 'https://api.github.com/repos/test/repo',
        repository: {
          full_name: 'test/repo',
