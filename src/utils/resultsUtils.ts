 ...
 * @returns Filtered array of items
 */
export const filterByStatus = (items: GitHubItem[], statusFilter: 'all' | 'open' | 'closed' | 'merged'): GitHubItem[] => {
...
export const filterByLabels = (
  ...
  return items.filter(item => {
    ...
    if (excludedLabels.length > 0 && item.labels?.some(l => excludedLabels.includes(l.name))) {
      return false;
    }
...
export const applyFiltersAndSort = (
  ...
  filteredItems = filterByLabels(filteredItems, filters.labelFilter, filters.excludedLabels);
